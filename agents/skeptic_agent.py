...
- **震荡偏强/偏多（强市）**：趋势延续性优先。
  ...
  ② 当存在明确的S级催化剂（政策文件/行业数据/公告订单/核心产品验证/龙头公司重大突破），且股价尚未完全反应时，"驱动逻辑"的质疑标准可考虑适度放宽（high→medium）。
  ③ **催化剂强度平衡**：若驱动逻辑有明确的S级催化剂支撑，即使存在2-3个medium风险（非high/veto），overall_verdict可考虑pass。但summary中必须注明未解决的风险。
- **震荡（中性）**：中性质疑，按实际情况判断。
